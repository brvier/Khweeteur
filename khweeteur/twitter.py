...
class Status(object):
  ...
  '''
    ...
    relative_created_at = property(GetRelativeCreatedAt,
                                   doc='Get a human readable string representingthe posting time'
                                   )
   ...
   '''
    ...
    '''
        ...
        return Status(
            ...
            text=unescape(data.get('text', None)),
    ...
    def __eq__(self, other):
        try:
            return other and self.id == other.id and self.name == other.name \
                and self.slug == other.slug and self.description \
                == other.description and self.full_name == other.full_name \
                and self.mode == other.mode and self.uri == other.uri \
                and self.member_count == other.member_count \
                and self.subscriber_count == other.subscriber_count \
                and self.following == other.following and self.user \
                == other.user
        except AttributeError:
...
class User(object):
  ...
  '''
    ...
    def __eq__(self, other):
        try:
            return other and self.id == other.id and self.name == other.name \
                and self.screen_name == other.screen_name and self.location \
                == other.location and self.description == other.description \
                and self.profile_image_url == other.profile_image_url \
                and self.profile_background_tile \
                == other.profile_background_tile \
                and self.profile_background_image_url \
                == other.profile_background_image_url \
                and self.profile_sidebar_fill_color \
                == other.profile_sidebar_fill_color \
                and self.profile_background_color \
                == other.profile_background_color and self.profile_link_color \
                == other.profile_link_color and self.profile_text_color \
                == other.profile_text_color and self.protected \
                == other.protected and self.utc_offset == other.utc_offset \
                and self.time_zone == other.time_zone and self.url == other.url \
                and self.statuses_count == other.statuses_count \
                and self.followers_count == other.followers_count \
                and self.favourites_count == other.favourites_count \
                and self.friends_count == other.friends_count and self.status \
                == other.status
        except AttributeError:
            return False
...
class DirectMessage(object):
  ...
  '''
    ...
    def __eq__(self, other):
        try:
            return other and self.id == other.id and self.created_at \
                == other.created_at and self.sender_id == other.sender_id \
                and self.sender_screen_name == other.sender_screen_name \
                and self.recipient_id == other.recipient_id \
                and self.recipient_screen_name == other.recipient_screen_name \
...
class Api(object):
  ...
  '''
    ...
    '''
        ...
        if username is not None and (access_token_key is None
        ...
        self.SetCredentials(username, password, access_token_key,
                            access_token_secret)
...
#    url = 'http://api.twitter.com/1/statuses/retweet/%s.json' % tweet_id
    ...
    '''
      ...
      # find the difference in time between now and the reset time + 1 hour
            ...
            delta = reset + datetime.timedelta(hours=1) \
    ...
    def _InitializeUserAgent(self):
        user_agent = 'Khweeteur-python-twitter/%s)' % __version__
...
class _FileCache(object):
    ...
    def _GetUsername(self):
        ...
        try:
            return os.getenv('USER') or os.getenv('LOGNAME') \
                or os.getenv('USERNAME') or os.getlogin() or 'nobody'
    ...
    def _GetPath(self, key):
        ...
        return os.path.join(self._root_directory, self._GetPrefix(hashed_key),
                            hashed_key)
...
if __name__ == '__main__':
    print 'test'
    s = \
        u'''тест тест тест тест тест тест тест тест тест тест тест тест тест тест тест тест тест тест тест тест тест тест тест тест тест тест тест тест тест  тест '''
    s2 = u'''тест тест тест тест тест тест'''
    api = Api(username='twitterfdghjuser', password='twittgfjker pass',
              access_token_key='the_key_g', access_token_secret='the_key_secret'
              )
    api.PostSerializedUpdates(s2)
